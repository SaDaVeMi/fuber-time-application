...
function requestsHandler(result) {
	...
	for (i = 0; i < result.length; i++) {
		...
		$("#requestList").append(
			'<li>' +
			'<p> Vacation Request id: ' + '#' + result[i].id + ' User: ' + '"'+ result[i].user.userName + '"' + '</p>' +
			...
			'<span style="display:inline-block; width: 2%;"></span>End: '  + splittedEnd[2] + '.' + splittedEnd[1] + '.' + splittedEnd[0] + ' ' +
